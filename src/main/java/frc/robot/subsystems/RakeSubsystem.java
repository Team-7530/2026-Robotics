...
public class RakeSubsystem extends SubsystemBase {
  ...
  public static final double RAKEINTAKE_KS = 0.0;
  public static final double RAKEINTAKE_KP = 8.0;
  public static final double RAKEINTAKE_KI = 0.2;
  public static final double RAKEINTAKE_KD = 0.001;
  ...
  private static final Distance flywheelDiameter = Inches.of(4);
  private static final Mass flywheelMass = Pounds.of(1);
  ...
  //    .withHorizontalZero(Degrees.of(0))
    ...
    private final Arm m_rakeArm = new Arm(m_rakeArmConfig);
      ...
      // YAMS controller and mechanism (initialized at declaration to match FlywheelSubsystem style)
    private final SmartMotorControllerConfig smc_riconfig = new SmartMotorControllerConfig(this)
        .withControlMode(ControlMode.CLOSED_LOOP)
        // PID Constants
    ...
    private final SmartMotorController m_rakeIntakeSMC = new TalonFXWrapper(m_rakeIntakeMotor, DCMotor.getKrakenX60Foc(1), smc_riconfig);
    ...
    public void periodic() {
      ...
      try {
        telemetry.putNumber("Rake Postion", this.getRakeArmPosition().in(Degrees));
      } catch (Exception e) {
    ...
    public Command setRakeIntakeVelocity(AngularVelocity speed) {
      return m_rakeIntake.setSpeed(speed);
    }

    public Command setRakeIntakeDutyCycle(double duty) {
      return m_rakeIntake.set(duty);
    }
  
     /**
     * Sets the rake intake motor speed
     *
     * @param duty double, target duty cycle
     */
    /**
     * Teleop controls
     *
    ...
    cspeed = MathUtil.applyDeadband(cspeed, STICK_DEADBAND);
      ...
      if ((rspeed != 0.0) || (cspeed != 0.0)) {
        ...
        m_rakeIntakeSMC.setDutyCycle(cspeed * kRakeIntakeTeleopFactor);
    } else if (m_isTeleop) {
      m_isTeleop = false;
